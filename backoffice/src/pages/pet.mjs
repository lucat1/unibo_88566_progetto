...
const PetWrapper = () => {
  const id = window.location.pathname.match(
    /\/pets\/([0-9a-fA-F]{8}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{12})$/
  )[1];
  const {
    data,
    loading: fetching,
...
const Pet = ({ id, data }) => {
  ...
  const patch = async (e) => {
    ...
    try {
      ...
      setError(err.message || "An error occourred while updating the pet");
    }
    setLoading(false);
  };
  const del = async (e) => {
    ...
    setError(null);
    try {
      await fetch(`store/pets/${id}`, withOptions("DELETE"));
      navigate("/pets");
    } catch (err) {
  ...
  return h(
    ...
    h(
      "section",
      { className: "column" },
      h("h1", { className: "is-size-3" }, "Pet #", data._id),
      h(
        "form",
        { onSubmit: patch },
        h(
