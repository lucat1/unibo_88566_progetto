<template>
  <div class="is-flex is-flex-direction-column is-justify-content-center is-align-items-center">
    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="350px" height="260px"
      viewBox="0 0 350 275" preserveAspectRatio="xMidYMid meet">
      <line x1="80" y1="257" x2="260" y2="257" class="piece"
        :class="strikes > 0 ? 'has-text-black' : 'has-text-grey-light'" />
      <line x1="100" y1="257" x2="100" y2="40" class="piece"
        :class="strikes > 1 ? 'has-text-black' : 'has-text-grey-light'" />
      <line x1="100" y1="40" x2="230" y2="40" class="piece"
        :class="strikes > 2 ? 'has-text-black' : 'has-text-grey-light'" />
      <line x1="100" y1="80" x2="130" y2="40" class="piece"
        :class="strikes > 3 ? 'has-text-black' : 'has-text-grey-light'" />
      <line x1="230" y1="40" x2="230" y2="70" class="piece"
        :class="strikes > 4 ? 'has-text-black' : 'has-text-grey-light'" />
      <circle cx="230" cy="90" r="20" class="piece" :class="strikes > 5 ? 'has-text-black' : 'has-text-grey-light'" />
      <line x1="230" y1="110" x2="230" y2="170" class="piece"
        :class="strikes > 6 ? 'has-text-black' : 'has-text-grey-light'" />
      <line x1="230" y1="140" x2="250" y2="120" class="piece"
        :class="strikes > 7 ? 'has-text-black' : 'has-text-grey-light'" />
      <line x1="230" y1="140" x2="210" y2="120" class="piece"
        :class="strikes > 8 ? 'has-text-black' : 'has-text-grey-light'" />
      <line x1="230" y1="170" x2="250" y2="200" class="piece"
        :class="strikes > 9 ? 'has-text-black' : 'has-text-grey-light'" />
      <line x1="230" y1="170" x2="210" y2="200" class="piece"
    ...
    <div class="letters my-4">
      <div class="letter" v-for="letter in wordDisplayLetters"
        :class="{ 'has-text-primary': letter.space || letter.letter != '' }">
        {{ letter.letter }}
      </div>
    </div>
    <div v-for="[start, end] in letterBounds">
      <button v-for="i in end - start" :disabled="state != 'p' || usedLetters.includes(letters[start + i - 1])"
        class="button has-text-grey-darker possibleLetter" :class="getStrikethroughClass(letters[start + i - 1])"
...
export default defineComponent({
  ...
  computed: {
    ...
    wordDisplayLetters() {
      return this.wordLetters.map((letter) => ({
        letter: "",
        space: letter == " ",
      }));
    },
    ...
    async tryLetter(letter) {
      ...
      for (let i = 0; i < this.wordDisplayLetters.length; i++)
        if (letter === this.wordLetters[i]) {
          this.wordDisplayLetters.splice(i, 1, { letter, space: false });
          match = true;
        }
      if (!match) ++this.strikes;
      const displayWord = this.wordDisplayLetters
        .map((letter) => (letter.space ? " " : letter.letter))
        .join("");
      ...
      } else if (this.strikes > PIECES) this.state = GameState.LOST;
    },
