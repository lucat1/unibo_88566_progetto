...
export default defineComponent({
  ...
  methods: {
    ...
    next() {
      if (this.selected < this.trivias.length - 1) this.selected++;
    },
  },
  watch: {
    async keyword(val) {
  ...
  computed: {
    ...
    async trivias() {
      if (this.pets.length - 1 > this.selected) return;
    },
  },
});
</script>
...
<template>
  <div v-if="trivias.length <= 0" class="is-flex" style="text-align: center">
    No trivias were found for the animal type
    ...
    <div class="card-content">
      <div class="content">
        ...
        <ul>
          ...
          <li v-if="trivias[selected].taxonomy.phylum">
            Phylum: {{ trivias[selected].taxonomy.phylum }}
          </li>
          <li v-if="trivias[selected].taxonomy.class">
            Class: {{ trivias[selected].taxonomy.class }}
          </li>
          <li v-if="trivias[selected].taxonomy.order">
            Order: {{ trivias[selected].taxonomy.order }}
          </li>
          <li v-if="trivias[selected].taxonomy.family">
            Family: {{ trivias[selected].taxonomy.family }}
          </li>
          <li v-if="trivias[selected].taxonomy.genus">
            Genus: {{ trivias[selected].taxonomy.genus }}
          </li>
          <li v-if="trivias[selected].taxonomy.scientific_name">
