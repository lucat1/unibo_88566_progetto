...
const useCart = (): [
  IItem[],
  (key: Key, i: IPet | IProduct, amount: number) => void,
  (key: Key, i: IProduct | IPet) => void,
  () => void
] => {
  ...
  const add = React.useCallback(
    (key: Key, i: IProduct | IPet, amount: number) =>
      setCart((items) => {
        let found = false;
        return items
          .reduce<IItem[]>((prev, item) => {
            if (item[key]?._id != i._id) return [...prev, item];
            else {
              found = true;
              return [
                ...prev,
                { ...item, amount: item.amount + amount } as IItem,
              ];
            }
          }, [])
  ...
  const del = React.useCallback(
    (key: Key, i: IProduct | IPet) =>
      setCart((items) => items.filter((item) => item[key]?._id != i._id)),
    [setCart]
  );
