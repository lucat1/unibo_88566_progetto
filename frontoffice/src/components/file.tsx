...
const File: React.FC<{ onUpload: (id: string) => void }> = ({ onUpload }) => {
  ...
  const handleChange: React.ChangeEventHandler<HTMLInputElement> = React.useCallback(async (e) => {
    console.log(e)
    setLoading(true)
    setError(null);
    ...
    <>
      <div className="file is-boxed is-fullwidth">
        <label className="file-label">
          <input disabled={loading} className="file-input" type="file" onChange={handleChange} />
      ...
      {error &&
        <span className="is-danger">{error}</span>
      }
    </>
  )
}
